...
void
ft_strncpy_test (char *dest, char *ft_dest, char *src, unsigned int n)
{
  char *res;
  char *ft_res;
  ...
  if (ft_res != ft_dest)
    printf (
        "> KO, return addres doesn't match *ft_dest, expected: %p, got: %p\n",
        ft_res, ft_dest);
  else if (strcmp (res, ft_res) != 0)
    printf (
        "> KO, string has not been copied properly, expected: %s, got: %s\n",
        res, ft_res);
  else
    printf ("> OK, result: %s\n", ft_res);
}
